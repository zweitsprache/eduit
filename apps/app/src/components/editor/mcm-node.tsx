...
export function normalizeMCMRowText(text: string) {
  return text.replace(
    /^Answer row ([A-Z]|\d+)$/,
    'Statement / Question row $1',
  );
}
  ...
  return (
    ...
    >
      <div className="custom-block__matrix-layout" ref={optionWidthRef}>
        ...
        />
        <BlockInstruction>
          {node.attrs.instruction || DEFAULT_BLOCK_INSTRUCTIONS.mcm}
        </BlockInstruction>
