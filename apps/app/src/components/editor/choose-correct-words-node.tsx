...
function generateOptions(
  ...
  const correctCount = Math.min(
    item.optionCount,
    Math.max(2, Math.round(item.optionCount * 0.3)),
  );
  ...
  useLayoutEffect(() => {
    ...
    const drawSolutions = () => {
      ...
      rows.querySelectorAll<HTMLElement>(
        '.choose-correct-words-node__option[data-correct="true"]:not([data-reference="true"])',
      ).forEach((option) => {
        ...
        const isExample = option.dataset.example === 'true';
        ...
        oval.dataset.solutionKind = isExample ? 'example' : 'solution';
            ...
            <div className="choose-correct-words-node__options">
              {options.map((option, optionIndex) => (
                <span
                  ...
                  data-reference={optionIndex === 0}
                  data-example={
                    attrs.showFirstAsExample
                    && itemIndex === 0
                    && optionIndex > 0
                    && option.correct
                    && !options.slice(1, optionIndex).some(
                      ({ correct }) => correct,
                    )
                  }
