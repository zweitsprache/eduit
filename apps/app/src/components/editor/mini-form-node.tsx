...
function MiniFormNodeView({ node, selected }: NodeViewProps) {
  ...
  const finalFieldStart = attrs.fields.length
    ? ((attrs.fields.length - 1) % attrs.columns) + 1
    : 1;
        ...
        {attrs.items.map((item, itemIndex) => (
          ...
          >
            ...
            <div className="mini-form-node__body">
              <p className="mini-form-node__prompt">
                {item.prompt.split('\n').map((line, lineIndex) => (
                  <Fragment key={`${item.id}-line-${lineIndex}`}>
                    {lineIndex > 0 && <br />}
                    {line}
                  </Fragment>
                ))}
              ...
              <div
                ...
                data-fill-remaining-row={attrs.fillRemainingRow}
                data-final-field-start={finalFieldStart}
              >
                {attrs.fields.map((field, fieldIndex) => {
                  ...
                  return (
                    <div
                      className="mini-form-node__field"
                      key={field.id}
                    >
  ...
  addAttributes() {
    return {
      ...
      fillRemainingRow: {
        default: false,
        parseHTML: (element) => (
          element.getAttribute('data-mini-form-fill-remaining-row') === 'true'
        ),
        renderHTML: (attributes) => ({
          'data-mini-form-fill-remaining-row': String(
            attributes.fillRemainingRow,
          ),
        }),
      },
      showFirstAsExample: {
        ...
        ({ commands }) =>
          commands.insertContent({
            ...
            attrs: {
              ...
              fillRemainingRow: attrs.fillRemainingRow ?? false,
